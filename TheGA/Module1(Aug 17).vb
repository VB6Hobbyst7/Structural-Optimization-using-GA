...
Module Module1
    ...
    Sub Main()
        ...
        For iter = 1 To n2
            ...
            End If
        Next
        Console.WriteLine("member property array generated")
        Console.WriteLine("Excel coming up")
        objApp.Visible = True
        ...
        For gen = 1 To 30
            ...
            Population_Array = Genetic_Algorithm_Application(Population_Array, obj, Active_sheet, Member_property, n_prismatic)
            ...
            For iter1 = 0 To (Population_Array.GetLength(0) - 1)
                ...
                For iter4 = 1 To Altered_Inertia.GetLength(0)
                    Active_sheet.Cells(20 + 2 * iter1, iter4) = Altered_Inertia(iter4 - 1, 0)
                    Active_sheet.Cells(21 + 2 * iter1, iter4) = Altered_Inertia(iter4 - 1, 1)
                Next
        ...
        objBooks = Nothing
        obj = Nothing
        Console.WriteLine("You made it! Man")
        Console.ReadKey()
    ...
    Private Function Binary_to_Real2(inertia_instance(,) As Population_class, u_limit As Double, L_limit As Double) As Double(,)
        ...
        For iter1 = 0 To inertia_instance.GetLength(0) - 1
            ...
            Next
            Console.Write("Decoded value @ Ix")
            Console.WriteLine(Decoded_value)
            Inertia = L_limit + ((diff / max_value) * Decoded_value)
            Console.Write("Diff:")
            Console.WriteLine(diff)
            Console.Write("L Limit:")
            Console.WriteLine(L_limit)
            Console.Write("U Limit:")
            Console.WriteLine(u_limit)
            Console.Write("Inertia:")
            Console.WriteLine(Inertia)
            Inertia_values(iter1, 0) = Inertia

            Decoded_value = 0
            For iter2 = (n_bits / 2) To n_bits - 1
                Decoded_value += inertia_instance(iter1, iter2).Value() * Pow(2, (n_bits - 1 - iter2))
            Next
            Console.Write("Decoded value @ Iy")
            Console.WriteLine(Decoded_value)
            Inertia = L_limit + ((diff / max_value) * Decoded_value)
            Console.Write("Diff:")
            Console.WriteLine(diff)
            ...
            Console.Write("Inertia:")
            Console.WriteLine(Inertia)
            Inertia_values(iter1, 1) = Inertia
        Next
        Console.WriteLine("Binary to real initiated")
    ...
    Public Function FitnessFunction(Population_Array(,) As Population_class, ByRef obj As Object, Active_sheet As _Worksheet, Member_property As String(), n_prismatic As Integer) As Double
        Console.WriteLine("You are in fitness function module")
        ...
        Console.WriteLine("Members are now defined")

        Dim MyThread As System.Threading.Thread

        For iter1 = 1 To (Population_Array.GetLength(0))
            'thickness input unit is meter
            If iter1 < Population_Array.GetLength(0) - n_prismatic + 1 Then
                thickness_in_m = (Real_thickness(iter1 - 1)) / 1000
                If Member_property(iter1 - 1) = "Square" Then
                    obj.Property.CreateTaperedTubeProperty(5, 0.0889, 0.0889, thickness_in_m)
                ElseIf Member_property(iter1 - 1) = "Pipe" Then
                    obj.Property.CreateTaperedTubeProperty(0, 0.2445, 0.2445, thickness_in_m)
                ElseIf Member_property(iter1 - 1) = "ISMB" Then
                    PropertyArrayISMB(1) = thickness_in_m
                    PropertyArrayISMB(4) = thickness_in_m
                    PropertyArrayISMB(6) = thickness_in_m
                    obj.Property.CreateTaperedIProperty(PropertyArrayISMB)
                ElseIf Member_property(iter1 - 1) = "Taper" Then
                    PropertyArrayTaper(1) = thickness_in_m
                    PropertyArrayTaper(4) = thickness_in_m
                    PropertyArrayTaper(6) = thickness_in_m
                ...
                End If
            Else
                If Member_property(iter1 - 1) = "Prismatic" Then
                    PropertyArrayGeneral(3) = Inertia_array(iter1 - (Population_Array.GetLength(0) - n_prismatic) - 1, 0)
                    PropertyArrayGeneral(4) = Inertia_array(iter1 - (Population_Array.GetLength(0) - n_prismatic) - 1, 1)
                    obj.Property.CreatePrismaticGeneralProperty(PropertyArrayGeneral)
                Else
                    Console.WriteLine("Not even prismatic")
                    Console.WriteLine(iter1)
                End If
            End If
        ...
        Next
        Console.WriteLine("Member assigned the new properties")
        obj.updateStructure()
        ...
        Threading.Thread.Sleep(360000.0)

        Console.WriteLine("waiting for analysis finished")

        MyThread = New System.Threading.Thread(AddressOf PressEnterAnalysis)
        MyThread.Start()

        Console.WriteLine("set interface mode 1")

        obj.View.SetInterfaceMode(1)

        Console.WriteLine("Interface mode is set")
        ...
        Console.WriteLine("Wait for 60 second is over")
        ...
        For iter1 = 1 To Population_Array.GetLength(0)
            ...
            Else
                ...
                Console.Write("wrong Ratio Recieved : ")
                Console.WriteLine(iter1)
            End If
        Next
        ...
        Row += 1
        Console.WriteLine("Going out of fitness function module")
    ...
    Public Function Function_Value(ratio_array() As Double) As Double
        ...
        Next
        Console.Write("Ratio Sum is:")
        Console.WriteLine(Ratio_Sum)
        Return (ratio_array.Length - Ratio_Sum)
    End Function


    'Genetic Algorithm Base application
    Public Function Genetic_Algorithm_Application(Population_Array(,,) As Population_class, ByRef obj As Object, Active_sheet As _Worksheet, Member_property As String(), n_prismatic As Integer) As Population_class(,,)
        ...
        Console.WriteLine("GA base function Initialised")
        Reproduced_Population = Reproduction_Roullete_Wheel(Population_Array, obj, Active_sheet, Member_property, n_prismatic)
    ...
    Private Function Simple_Crossover(reproduced_Population(,,) As Population_class, Crossover_Probability As Double) As Population_class(,,)
        ...
        Dim Crossed_population(reproduced_Population.GetLength(0) - 1, reproduced_Population.GetLength(1) - 1, reproduced_Population.GetLength(2) - 1) As Population_class
        ...
        While Deciding_variable
            ...
            If random_number1 <> random_number2 Then
                If (Already_taken(random_number1) = False) And (Already_taken(random_number2) = False) Then
                    ...
                    Next
                    population_count += 2
                    If population_count >= Crossover_population Then
                        Deciding_variable = False
                    End If

                ElseIf Already_taken(random_number1) = True And Already_taken(random_number2) = False Then
                ...
                ElseIf Already_taken(random_number1) = False And Already_taken(random_number2) = True Then
                    While Inner_decision_variable
                        Extra_random_number = Int(Rnd() * Total_population)
        ...
        'Defining 1000 point crossover, iter2 below will change, caution
        Dim Crossover_points(999) As Integer
        Dim string_length As Long
        string_length = (reproduced_Population.GetLength(1)) * (reproduced_Population.GetLength(2))

        For iter1 = 0 To (Crossover_points.Length() - 1)
        ...
        For iter1 As Integer = 0 To (To_be_crossed_population.GetLength(0) - 1) Step 2
            ...
            'Cautioun, iter2 will change depending on even and odd nature of number of crossover point
            For iter2 As Integer = 1 To (Crossover_points.Length() - 1) Step 2

                For iter3 As Integer = Crossover_points(iter2 - 1) To Crossover_points(iter2)

                    Temporary_instance = To_be_crossed_population(iter1, Int(iter3 \ 8), iter3 Mod 8)
                    To_be_crossed_population(iter1, Int(iter3 \ 8), iter3 Mod 8) = To_be_crossed_population(iter1 + 1, Int(iter3 \ 8), iter3 Mod 8)
                    To_be_crossed_population(iter1 + 1, Int(iter3 \ 8), iter3 Mod 8) = Temporary_instance
        ...
        For iter1 = 0 To (reproduced_Population.GetLength(0) - 1)
            ...
            End If
        Next
        If count = Crossover_population Then
            Console.WriteLine("Crossover is fine, Returning the crossed population")
        End If
    ...
    Private Function Reproduction_Roullete_Wheel(population_Array(,,) As Population_class, ByRef obj As Object, Active_sheet As _Worksheet, Member_property As String(), n_prismatic As Integer) As Population_class(,,)
        ...
        For iter1 = 0 To (population_Array.GetLength(0) - 1)
            ...
            Fitness_values(iter1) = FitnessFunction(Population_Instance, obj, Active_sheet, Member_property, n_prismatic)
    ...
    Private Sub PressEnterResults()
        ...
        Threading.Thread.Sleep(10000)
        Console.WriteLine("We are in the thread")
        FocusWindow("Results Setup", Nothing)
        Threading.Thread.Sleep(5000)
        SendKeys.SendWait("{ENTER}")
        Console.WriteLine("Enter entered")
